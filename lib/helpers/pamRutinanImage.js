...
const candidatePublicDirs = () => {
  ...
  const fromEnvRoot = process.env.APP_ROOT_PATH
    ? path.join(process.env.APP_ROOT_PATH, "public")
    : null;
  const fromEnvPublic = process.env.UPLOAD_PUBLIC_DIR || null;

  return [
    fromEnvPublic,
    fromEnvRoot,
    path.join(process.cwd(), "public"),
    path.join(inferredProjectRoot, "public"),
  ].filter(Boolean);
};
...
export const savePamRutinanPhotoFromDataUrl = async ({ dataUrl, tenantKey = "default" }) => {
  ...
  const uploadsAbsDir = path.join(publicDir, uploadsRelativeDir);
