...
const resolveUploadStorageDir = async () => {
...
export const saveFinanceReceiptPhotoFromDataUrl = async ({ dataUrl, tenantKey = "default" }) => {
	...
	const storageRootDir = await resolveUploadStorageDir();
	const uploadsRelativeDir = path.join("finance-receipt", safeTenant);
...
export const deleteFinanceReceiptPhotoIfExists = async (publicUrl) => {
	const normalized = String(publicUrl || "").trim();
	if (!normalized.startsWith("/uploads/finance-receipt/")) {
		return;
	}
