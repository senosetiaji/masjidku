	...
	try {
		...
		if (periodStart) {
			const [priorIncome, priorExpense] = await Promise.all([
				prisma.pamKas.aggregate({ where: { userId, date: { lt: periodStart }, type: "income" }, _sum: { amount: true } }),
				prisma.pamKas.aggregate({ where: { userId, date: { lt: periodStart }, type: "expense" }, _sum: { amount: true } }),
			]);
