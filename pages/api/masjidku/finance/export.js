...
export default async function handler(req, res) {
	...
	try {
		...
		const [sumIncome, sumExpense, rows] = await Promise.all([
			prisma.keuangan.aggregate({ where: { ...baseWhere, type: "income" }, _sum: { amount: true } }),
			prisma.keuangan.aggregate({ where: { ...baseWhere, type: "expense" }, _sum: { amount: true } }),
		...
		const filenameParts = ["keuangan-masjid"];
		...
		doc.fontSize(16).text("Laporan Keuangan Masjid", { align: "center" });
		doc.moveDown(0.3);
		const metaLines = [];
