...
export default async function handler(req, res) {
	...
	try {
		...
		const animalTypeCandidate = (normalizeQueryValue(req.query.animal_type ?? req.query.animalType) || "").toString().trim().toLowerCase();
		...
		const allowedTypes = ["hewan", "uang"];
		const typeFilter = allowedTypes.includes(typeCandidate) ? typeCandidate : null;

		const allowedAnimalTypes = ["kambing", "sapi", "domba"];
		const animalTypeFilter = allowedAnimalTypes.includes(animalTypeCandidate) ? animalTypeCandidate : null;
			...
			...(search
				? {
					OR: [
						...
						{ animalType: { contains: search } },
					],
				}
				: {}),
			...(dateFilter ? { date: dateFilter } : {}),
			...(typeFilter ? { type: typeFilter } : {}),
			...(animalTypeFilter ? { animalType: animalTypeFilter } : {}),
		};

		const total = await prisma.qurban.count({ where });

		const rows = await prisma.qurban.findMany({
			where,
		...
		console.error("GET QURBAN ERROR:", error);
