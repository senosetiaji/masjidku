...
export default async function handler(req, res) {
	...
	try {
		...
		const [countAll, sumHewanAmount, sumUangPrice] = await Promise.all([
			prisma.qurban.count({ where: baseWhere }),
			prisma.qurban.aggregate({ where: { ...baseWhere, type: "hewan" }, _sum: { amount: true } }),
			prisma.qurban.aggregate({ where: { ...baseWhere, type: "uang" }, _sum: { price: true } }),
		]);

		return res.status(200).json({
			status: 200,
			message: "qurban_summary_fetched",
			data: {
				totalQurban: countAll,
				totalHewan: sumHewanAmount._sum.amount || 0,
				totalUang: sumUangPrice._sum.price || 0,
			},
		});
	} catch (error) {
		console.error("GET QURBAN SUMMARY ERROR:", error);
