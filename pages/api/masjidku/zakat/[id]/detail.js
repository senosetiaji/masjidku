...
export default async function handler(req, res) {
	...
	try {
		...
		const zakat = await prisma.zakats.findUnique({
			where: { id },
			select: {
				id: true,
				name: true,
		...
		return res.status(200).json({
			status: 200,
			message: "zakat_detail_fetched",
			data: {
				id: zakat.id,
				name: zakat.name,
				date: zakat.date.toISOString(),
				type: zakat.type,
				zakatType: zakat.zakatType,
				amount: zakat.amount,
				description: zakat.description,
				createdAt: zakat.createdAt.toISOString(),
				updatedAt: zakat.updatedAt.toISOString(),
			},
		});
	} catch (error) {
		console.error("GET ZAKAT DETAIL ERROR:", error);
