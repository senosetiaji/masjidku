...
export default async function handler(req, res) {
	...
	try {
		...
		await prisma.penerimaZakat.delete({ where: { id } });

		return res.status(200).json({ status: 200, message: "penerima_zakat_deleted" });
	} catch (error) {
		console.error("DELETE PENERIMA ZAKAT ERROR:", error);
		if (error?.code === "P2025") {
